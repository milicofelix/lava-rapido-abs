    ...
    public function __invoke(): RedirectResponse|View
    {
        ...
        return view('app.dashboard', [
            ...
            'executiveComparisons' => [
                'washOrders' => $this->comparisonLabel($monthlyWashOrders, $previousMonthlyWashOrders, 'mês anterior'),
                'revenue' => $this->comparisonLabel((float) $monthlyRevenue, (float) $previousMonthlyRevenue, 'mês anterior'),
                'ticketAverage' => $this->comparisonLabel((float) $monthlyTicketAverage, (float) $previousMonthlyTicketAverage, 'mês anterior'),
                'recurringCustomers' => $this->comparisonLabel(count($monthlyRecurringCustomers), count($previousMonthlyRecurringCustomers), 'mês anterior'),
            ],
            ...
            ->map(function (array $column) use ($washOrders) {
                ...
                return [
                    ...
                    'orders' => $orders->take(3)->map(fn (WashOrder $washOrder) => [
                        ...
                        'service' => $washOrder->services->first()?->pivot->service_name ?? 'Serviço',
