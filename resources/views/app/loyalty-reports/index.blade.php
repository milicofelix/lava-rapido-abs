<x-app.layout heading="Fidelidade" title="Fidelidade · AutoFlow">
    <div class="space-y-5">
        <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm" data-tour="loyalty-header">
            <div class="flex flex-wrap items-start justify-between gap-4">
                ...
                <div class="flex flex-wrap items-center gap-2" data-tour="loyalty-actions">
            ...
            <form method="GET" class="mt-5 grid gap-3 lg:grid-cols-[160px_160px_1fr_1fr_190px_auto]" data-tour="loyalty-filters">
                <label class="block">
        ...
        <section class="grid gap-3 md:grid-cols-2 xl:grid-cols-4" data-tour="loyalty-metrics">
        ...
        <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm" data-tour="loyalty-retention">
            ...
            <div class="mt-5 grid gap-5 xl:grid-cols-[0.8fr_1.2fr]">
                <div class="rounded-2xl border border-slate-100 p-4" data-tour="loyalty-frequency">
                ...
                <div class="rounded-2xl border border-slate-100 p-4" data-tour="loyalty-monthly">
        ...
        <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm" data-tour="loyalty-near-reward">
        ...
        <div class="grid gap-5 xl:grid-cols-[0.9fr_1.1fr]">
            <section class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm" data-tour="loyalty-customer-progress">
            ...
            <section class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm" data-tour="loyalty-coupons">
                <div class="border-b border-slate-200 px-5 py-4">
                ...
                <div class="divide-y divide-slate-100">
                    @forelse ($coupons as $coupon)
                        @php
                            $statusTone = match ($coupon->effectiveStatus()) {
    ...
    @php
        $loyaltyTour = [
            'key' => 'loyalty-reports.index.v1',
            'title' => 'Entendendo a Fidelidade',
            'steps' => [
                [
                    'target' => '[data-tour="loyalty-header"]',
                    'title' => 'Programa de fidelidade',
                    'body' => 'Aqui você acompanha se o programa está ativo, quantos cupons foram gerados e como os clientes estão evoluindo para ganhar benefícios.',
                ],
                [
                    'target' => '[data-tour="loyalty-actions"]',
                    'title' => 'Ações rápidas',
                    'body' => 'Use Campanhas para falar com grupos de clientes, Processar cupons para gerar benefícios pendentes e Exportar CSV para conferência externa.',
                ],
                [
                    'target' => '[data-tour="loyalty-filters"]',
                    'title' => 'Filtros do relatório',
                    'body' => 'Filtre por período, cliente, código, telefone ou status do cupom para investigar casos específicos sem misturar toda a base.',
                ],
                [
                    'target' => '[data-tour="loyalty-metrics"]',
                    'title' => 'Indicadores de cupons',
                    'body' => 'Estes cards resumem cupons ativos, usados, expirados e o valor de desconto concedido no período selecionado.',
                ],
                [
                    'target' => '[data-tour="loyalty-retention"]',
                    'title' => 'Retenção e recorrência',
                    'body' => 'Esta área mostra se os clientes estão voltando, quantos repetiram lavagens e a média de visitas por cliente.',
                ],
                [
                    'target' => '[data-tour="loyalty-frequency"]',
                    'title' => 'Frequência',
                    'body' => 'Veja quantos clientes fizeram uma, duas, três ou mais lavagens no período. Ajuda a entender o comportamento de retorno.',
                ],
                [
                    'target' => '[data-tour="loyalty-monthly"]',
                    'title' => 'Evolução mensal',
                    'body' => 'Compare mês a mês o volume de lavagens, clientes atendidos e taxa de recorrência.',
                ],
                [
                    'target' => '[data-tour="loyalty-near-reward"]',
                    'title' => 'Perto do benefício',
                    'body' => 'Clientes próximos de ganhar cupom aparecem aqui. É uma boa lista para atendimento, lembrete ou campanha manual.',
                ],
                [
                    'target' => '[data-tour="loyalty-customer-progress"]',
                    'title' => 'Progresso individual',
                    'body' => 'Confira a contagem de lavagens válidas por cliente e se ele já possui cupom ativo.',
                ],
                [
                    'target' => '[data-tour="loyalty-coupons"]',
                    'title' => 'Cupons do período',
                    'body' => 'A lista mostra código, cliente, benefício e status. Abra o cupom para ver a versão personalizada e compartilhar com o cliente.',
                ],
            ],
        ];
    @endphp
    <script type="application/json" data-onboarding-tour>
        {!! json_encode($loyaltyTour, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES) !!}
    </script>
</x-app.layout>
