<x-app.layout heading="Editar cliente" title="Editar cliente · AutoFlow">
    <div class="grid min-w-0 gap-5 xl:grid-cols-[minmax(0,1fr)_360px]">
        <form method="POST" action="{{ route('customers.update', $customer) }}" class="min-w-0">
            @csrf
            @method('PUT')
        ...
        <aside class="min-w-0 space-y-4">
            <section class="min-w-0 overflow-hidden rounded-2xl border border-fuchsia-200 bg-fuchsia-50 p-4 shadow-sm sm:p-5">
                ...
                @if ($loyaltyProgress['enabled'])
                    @if ($loyaltyProgress['has_active_coupon'])
                        <div class="mt-4 min-w-0 rounded-2xl border border-emerald-200 bg-emerald-50 p-4">
                    ...
                    <dl class="mt-4 grid gap-3">
                        <div class="min-w-0 rounded-2xl bg-white p-4">
                        ...
                        <div class="min-w-0 rounded-2xl bg-white p-4">
                            ...
                            <dd class="mt-1 break-words font-black text-slate-950">{{ $loyaltyProgress['label'] }}</dd>
                        </div>
                    </dl>
                @else
            ...
            <section class="min-w-0 overflow-hidden rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
                ...
                <div class="mt-4 space-y-3">
                    @forelse ($loyaltyCoupons as $coupon)
                        <a href="{{ route('loyalty-coupons.show', $coupon) }}" class="block min-w-0 rounded-2xl border border-slate-200 bg-slate-50 p-3 transition hover:border-fuchsia-200 hover:bg-fuchsia-50">
                            <div class="flex min-w-0 items-start justify-between gap-3">
                                <div class="min-w-0">
                                    <p class="truncate font-black tracking-wide text-slate-950">{{ $coupon->code }}</p>
                                    <p class="mt-1 break-words text-xs text-slate-500">{{ $coupon->benefitLabel() }}</p>
